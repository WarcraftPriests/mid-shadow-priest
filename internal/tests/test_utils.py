...
def test_get_simc_dir_dungeons_with_talent():
    assert (
        utils.get_simc_dir("archon", "profiles", dungeons=True)
        == "profiles/dungeons/archon/"
    )
    assert (
        utils.get_simc_dir("archon", "output", dungeons=True)
        == "output/dungeons/archon/"
    )
...
def test_cleanup_isolation(tmp_path):
    ...
    (composite_dir / "base_pw_sa_1.simc").write_text(
        "composite profile", encoding="utf8"
    )
    ...
    assert not (composite_dir / "base_pw_sa_1.simc").exists(), (
        "composite file should be removed"
    )
    assert (dungeons_dir / "base_slice.simc").exists(), (
        "dungeons file should be untouched"
    )
    ...
    assert (composite_dir / "base_pw_sa_1.simc").exists(), (
        "composite file should be untouched"
    )
    assert not (dungeons_dir / "base_slice.simc").exists(), (
        "dungeons file should be removed"
    )
